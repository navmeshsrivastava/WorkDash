...
exports.updateTaskSolution = async (req, res) => {
  try {
    const { taskId } = req.params;
    const { solution } = req.body;
...
exports.undoTaskSubmission = async (req, res) => {
  try {
    const { taskId } = req.params;
    const userId = req.user.id;
...
exports.deleteTask = async (req, res) => {
  try {
    ...
    if (!task) return res.status(404).json({ error: 'No such task exists' });
...
exports.getTasksDoneByUser = async (req, res) => {
  try {
    const { userId } = req.params;
